...
**goalkeeper** is a Stop-hook guard for Claude Code & Codex. It won't let this
session end until the goals it is holding are provably complete. Every time the
agent tries to stop, goalkeeper re-injects the unfinished checklist as the next
instruction — so the agent keeps working instead of quitting early.

**Commands**
- `/goalkeeper:go <objective>` — decompose an objective into goals and start working
