---
description: Arm goalguard with an objective — decompose it into verifiable goals, then work until done.
argument-hint: <objective to complete>
---

The user wants goalguard to hold this session to an objective:
...
2. **Record** each goal:
   `node "${CLAUDE_PLUGIN_ROOT}/hooks/goalguard-cli.js" add "<goal text>"`

3. **Confirm the mode is armed.** If `goalguard-cli.js status` reports
   `mode=off`, set it: `goalguard-cli.js mode standard` (or `strict` if the
...
4. **Start working immediately.** Do not stop to ask permission. As each goal
   ...
   once you have concrete proof). goalguard will block any attempt to stop
