---
name: goalguard
description: Use when the user wants an agent to keep working until a task is fully and verifiably complete, to run autonomously for a long time, or to stop quitting early. Explains how to arm goalguard, decompose objectives into verifiable goals, and check them off so the Stop guard releases.
---

# goalguard

goalguard is a continuation guard. It uses the Claude Code **Stop hook** to
...
1. **Arm** with goals. Either the user runs `/goalguard <objective>` (which asks
   you to decompose it), or you record goals yourself as you discover them:

   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/hooks/goalguard-cli.js" add "all tests in src/ pass"
   ```
...
3. **Check off** each goal the moment it is genuinely finished:
   ...
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/hooks/goalguard-cli.js" done g2
   ```
...
Set with `goalguard-cli.js mode <name>` or the `GOALGUARD_DEFAULT_MODE` env var.
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/hooks/goalguard-cli.js" verify g2
```

If the double-check fails, reopen it and fix it:

```bash
node "${CLAUDE_PLUGIN_ROOT}/hooks/goalguard-cli.js" reopen g2
```

This is how goalguard turns "the model said it's done" into "the model proved
...
- If you are truly stuck, the loop budget (`GOALGUARD_MAX_LOOPS`, default 30
...
State is plain JSON at `<project>/.goalguard/state.json` — safe to read, edit,
or delete by hand. `goalguard-cli.js status` prints the current mode and
checklist.
